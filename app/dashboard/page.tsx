    ...
    <main className="mx-root">
      ...
      <header className="mx-header"><div className="mx-header-inner">
        ...
        <div style={{display:"flex", gap:8, alignItems:"center"}}>
          <span style={{fontFamily:"JetBrains Mono", fontSize:12.5, color:"rgba(255,255,255,0.65)"}}>{user.email}</span>
      ...
      <div style={{maxWidth:900, margin:"0 auto", padding:"32px 24px", fontFamily:"JetBrains Mono"}}>
        ...
        <div style={{border:"1px solid rgba(0,255,136,0.25)", background:"rgba(10,10,10,0.95)", padding:20, marginBottom:16}}>
          <div style={{fontWeight:900, fontSize:14.5, marginBottom:4}}>[PERFORMANCE_MATRIX]</div>
          <div style={{fontSize:12.5, color:"rgba(255,255,255,0.62)", marginBottom:16, lineHeight:1.65}}>
          ...
          {!overallStats ? (
            ...
            <>
              <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(120px,1fr))", gap:12, marginBottom:18}}>
                <div>
                  <div style={{fontSize:10.5, color:"rgba(255,255,255,0.55)"}}>TOTAL SIGNAL CLOSED</div>
                  <div style={{fontSize:22, fontWeight:900, color:"#fff"}}>{overallStats.total_closed}</div>
                </div>
                <div>
                  <div style={{fontSize:10.5, color:"rgba(255,255,255,0.55)"}}>WIN RATE</div>
                  <div style={{fontSize:22, fontWeight:900, color:"#00FF88"}}>{overallStats.win_rate_pct}%</div>
                </div>
                <div>
                  <div style={{fontSize:10.5, color:"rgba(255,255,255,0.55)"}}>WIN / LOSS</div>
                  <div style={{fontSize:22, fontWeight:900, color:"#fff"}}>{overallStats.wins} / {overallStats.losses}</div>
                </div>
                <div>
                  <div style={{fontSize:10.5, color:"rgba(255,255,255,0.55)"}}>SYMBOLS TRACKED</div>
                  <div style={{fontSize:22, fontWeight:900, color:"#fff"}}>{overallStats.symbols_tracked}</div>
                </div>
              </div>

              <div style={{fontSize:11.5, fontWeight:800, color:"rgba(255,255,255,0.65)", marginBottom:8, letterSpacing:"0.05em"}}>BREAKDOWN PER TIER SINYAL</div>
        ...
        <div style={{border:"1px solid rgba(255,255,255,0.12)", background:"rgba(10,10,10,0.9)", padding:20, marginBottom:16}}>
          <div style={{fontWeight:900, fontSize:14.5, marginBottom:6}}>DOWNLOAD_EA.EX5</div>
          <div style={{fontSize:12.5, color:"rgba(255,255,255,0.58)", marginBottom:16}}>Satu file untuk semua tier. Tier auto dari license key.</div>
          <a href={eaUrl} target="_blank" className="mx-btn-solid mx-glow-green" style={{display:"inline-block", padding:"10px 18px", fontSize:13, textDecoration:"none"}}>▼ Download EA</a>
          <div style={{fontSize:10.5, color:"rgba(255,255,255,0.6)", marginTop:10, wordBreak:"break-all"}}>{eaUrl}</div>
          ...
          <div style={{marginTop:14, border:"1px solid rgba(0,255,136,0.3)", background:"rgba(0,255,136,0.05)", padding:12}}>
            <div style={{display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:8}}>
              ...
              <span style={{color:"#00FF88", fontWeight:900, fontSize:12.5}}>VIRUSTOTAL VERIFIED</span>
              <span style={{fontSize:11.5, color:"rgba(255,255,255,0.62)"}}>0 / 60 engine mendeteksi ancaman</span>
            </div>
            <div style={{fontSize:11, color:"rgba(255,255,255,0.58)", wordBreak:"break-all", marginBottom:6}}>
              SHA-256: 38811b7bcac1c703288cfb37f8fde2a40a26f5dcd28700d282509e28c6ee1287
            </div>
            <a href="https://www.virustotal.com/gui/file/38811b7bcac1c703288cfb37f8fde2a40a26f5dcd28700d282509e28c6ee1287/detection" target="_blank" rel="noopener noreferrer" style={{fontSize:11.5, color:"#00FF88"}}>
        ...
        {/* KLAIM */}
        <div style={{border:"1px solid rgba(0,255,136,0.25)", background:"rgba(10,10,10,0.95)", padding:20, marginBottom:16}}>
          <div style={{fontWeight:900, fontSize:14.5, marginBottom:6}}>KLAIM LICENSE BARU</div>
          <div style={{fontSize:12.5, color:"rgba(255,255,255,0.58)", marginBottom:16}}>Masukkan key dari admin.</div>
          ...
          <div style={{marginTop:12, fontSize:12.5, color:"rgba(255,255,255,0.62)"}}>
        ...
        <div style={{border:"1px solid rgba(255,255,255,0.12)", background:"rgba(10,10,10,0.9)", padding:20}}>
          <div style={{fontWeight:900, fontSize:14.5, marginBottom:12}}>LICENSE SAYA [{licenses.length}]</div>
          {licenses.length===0 ? (
            <div style={{border:"1px dashed rgba(255,255,255,0.2)", padding:12, fontSize:12.5, color:"rgba(255,255,255,0.62)"}}>[ ] Belum ada license</div>
          ) : (
            <div style={{display:"flex", flexDirection:"column", gap:8}}>
              {licenses.map((l:any,i:number)=>(
                <div key={i} style={{border:"1px solid rgba(0,255,136,0.2)", padding:12, display:"flex", justifyContent:"space-between", alignItems:"center", background:"#0a0a0a"}}>
                  <div>
                    <div style={{color:"#00FF88", fontSize:14.5, fontWeight:700}}>{l.license_key}</div>
                    <div style={{fontSize:11.5, color:"rgba(255,255,255,0.55)", marginTop:4}}>Tier: {l.tier} | Exp: {l.expired_at? new Date(l.expired_at).toLocaleDateString(): "Lifetime"} | Status: {l.status}</div>
                  </div>
                  <span style={{border:"1px solid rgba(0,255,136,0.3)", padding:"4px 8px", fontSize:11.5}}>{l.tier}</span>
                </div>
              ))}
            </div>
          )}
